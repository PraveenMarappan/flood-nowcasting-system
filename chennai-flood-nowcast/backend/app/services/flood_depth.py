...
class FloodDepthEngine:
    ...
    def compute_flood_depth(
        ...
        """
        # 1. 0-Rainfall Recovery Check at current time (no forecast offset, no prior depth)
        if rainfall_rate_mm_hr <= 0 and forecast_offset_minutes == 0 and not current_water_depth_cm:
        ...
        base_depth_cm = current_water_depth_cm if current_water_depth_cm is not None else (runoff_depth_cm * ponding_factor)
